...
/*   Updated: 2018/10/11 18:03:24 by dkotlyar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		check_resid(char **residue, char **buff, char **line)
{
	...
	{
		tmp = ft_strnew(ft_strlen(*line) + start - *buff);
		ft_strcpy(tmp, *line);
		free(*line);
		ft_strncat(tmp, *buff, start - *buff);
		*line = tmp;
		tmp = ft_strnew(ft_strlen(*buff) - (start - *buff) - 1);
